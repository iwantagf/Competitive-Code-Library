...
struct Node {
	...
	long long sum, lazy;
	bool islazy;
	Node(int lo, int hi): l(lo), r(hi), islazy(0) {
		left = right = -1;
		sum = lazy = 0;
	}
};
...
struct STMAX {
	...
	void down(int r) {
		if (T[r].islazy) {
			T[T[r].left].lazy = T[T[r].right].lazy = T[r].lazy;
			T[T[r].left].islazy = T[T[r].right].islazy = 1;
			T[r].islazy = 0;
		}
	}
	...
	void up(int r) {
		...
		int mid = (lo + hi)/2;
		long long Left = (T[T[r].left].islazy) ? T[T[r].left].lazy * (mid - lo + 1) : T[T[r].left].sum;
		long long Right = (T[T[r].right].islazy) ? T[T[r].right].lazy * (hi - mid) : T[T[r].right].sum;
		T[r].sum = Left + Right;
	}
	...
	long long Get(int r, int u, int v) {
		...
		if (u <= lo && hi <= v) {
			return (T[r].islazy) ? T[r].lazy * (hi - lo + 1) : T[r].sum;
		}
